#!/usr/bin/env bash
#
# Build neural binaries for all platforms locally.
...
#     neural-darwin-arm64.tar.gz
#     neural-darwin-x64.tar.gz
#     neural-linux-x64.tar.gz
#     neural-linux-arm64.tar.gz
#     neural-windows-x64.zip
...
if [[ "$SKIP_DEPS" == "false" ]]; then
    ...
    npm install --no-save --force \
        @hyperspaceng/neural-clipboard-darwin-arm64@0.3.0 \
        @hyperspaceng/neural-clipboard-darwin-x64@0.3.0 \
        @hyperspaceng/neural-clipboard-linux-x64-gnu@0.3.0 \
        @hyperspaceng/neural-clipboard-linux-arm64-gnu@0.3.0 \
        @hyperspaceng/neural-clipboard-win32-x64-msvc@0.3.0 \
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/neural.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/neural
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating neural-$platform.zip..."
        (cd $platform && zip -r ../neural-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating neural-$platform.tar.gz..."
        mv $platform neural && tar -czf neural-$platform.tar.gz neural && mv neural $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../neural-$platform.zip)
    else
        tar -xzf neural-$platform.tar.gz && mv neural $platform
    fi
done
